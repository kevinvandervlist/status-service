...
export function task(taskname: string, option?: string):any {
  util.log("Loading task", chalk.yellow(taskname, option || ""));
  return require(join("..", "tasks", taskname))(gulp, gulpLoadPlugins(), option);
}

export function runSequence(...sequence: any[]):any {
  let tasks = [];
  let _sequence = sequence.slice(0);
  sequence.pop();

  scanDir(TASKS_PATH, (taskname:any) => tasks.push(taskname));

  sequence.forEach((task:any) => {
...
function scanDir(root: string, cb: (taskname: string) => void):void {
  if (!existsSync(root)) {
    return;
  }
